...
As
UPDATE Course
SET accepted = 'True' , adminId=@adminid
Where id = @courseId
...
RAISERROR('Course Not in Your Assigned Courses', 16, 1)
go
...
RAISERROR('Can not update Grade', 16, 1)
go
...
RAISERROR('Can not Issue Certificate!', 16, 1)
...
From Course LEFT OUTER JOIN StudentTakeCourse ON Course.id = StudentTakeCourse.cid 
...
ON STC.cid = C.id INNER JOIN Assignment A ON STC.assignmentNumber = A.number 
WHERE STC.cid = @cid AND STC.sid = @sid AND C.id = @cid AND STC.assignmenttype = @assignType 
...
END
go
create proc ExistPromoCode
@input varchar(6),
@result int output
As
select @result=count() from Promocode 
where code=@input


go
create proc ExistStudent
@StudentId int,
@counter int output
As
select @counter=count(*) from Student 
where Student.id=@StudentId

go
create proc ShowAllPromo
as
select * from Promocode

go
create proc ExistMultiPromo
@promo varchar(6),
@sid int ,
@result int output
As
select @result=count(*) from StudentHasPromocode 
where StudentHasPromocode.sid=@sid and StudentHasPromocode.code=@promo

go
Create proc getid
@resultid int output
AS 
select @resultid= a.id from
(SELECT TOP 1 * 
FROM Users 
ORDER BY Users.id DESC) a
-----------------------------------new added
go 
create proc getCourseID
@coursename varchar(20) ,
@courseID int output 
as
select @courseID =Course.id  from Course
